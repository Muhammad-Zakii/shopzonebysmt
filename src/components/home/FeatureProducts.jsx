        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mx-10">
          {data.slice(0, 8).map((product) => (
            <Link to={`/product/${product.id}`} key={product.id}>
              <div className="relative rounded-lg overflow-hidden bg-white border shadow-lg hover:shadow-xl transition-transform transform hover:scale-105 duration-300 h-[400px] cursor-pointer">
                <img
                  src={product.image[0]}
                  alt={product.heading}
